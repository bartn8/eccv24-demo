...
class MID70FrameWrapper(DeviceFrameWrapper):
    def __init__(self, frame_depth, frame_reflectivity, depth_scale = 1, start_scan_timestamp = 0, end_scan_timestamp = 0, diff=0) -> None:
        super().__init__({"depth": frame_depth, "reflectivity": frame_reflectivity, "depth_scale": depth_scale}, ("depth", "reflectivity", "depth_scale"), diff)

        self.start_scan_timestamp = start_scan_timestamp
        self.end_scan_timestamp = end_scan_timestamp
        
    def get_timestamp(self, key=None):
        if key is None or key not in self.keys:
            return None
        
        #return (self.end_scan_timestamp-self.start_scan_timestamp) / 1e9
        return (self.end_scan_timestamp) / 1e9
